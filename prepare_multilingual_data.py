...
parser.add_argument('--dataset', '-d', metavar='dataset', type=str, default='', help='dataset type, [iwslt]')
...
input_path = args.dataset+args.year
...
output_path = args.dataset+args.year+'.'
...
for pair in l_pairs:
    ...
    for filename in filenames:
            ...
            with open(input_path+'.'+pair[0]+'-'+pair[1]+'/'+filename+'.'+pair[0]+'-'+pair[1]+'.'+pair[0],'r', encoding="utf-8") as input:
                result = list()
            ...
            with open(input_path+'.'+pair[0]+'-'+pair[1]+'/'+filename+'.'+pair[0]+'-'+pair[1]+'.'+pair[1],'r', encoding="utf-8") as input:
                result = list()
                for i, line in enumerate(input):
                    str_encoding = list(map(str, line.strip().encode('utf-8')))
                    str_ = ' '.join(str_encoding)+'\n'
