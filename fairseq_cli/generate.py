...
def _main(args, output_file):
    ...
    def decode_fn_tgr(x):
        ...
        xx = [str(t) if (t != '<unk>' and t != '<<unk>>' and not t.startswith('madeupword')) else '32' for t in x.split(' ')]
        print('11111111111111111111111111111111111111', x)
        if x != '':
            m = bytes(map(int, xx)).decode('utf-8', 'ignore')
        else:
            m = 'none'
        return m
    ...
    def decode_fn_src(x):
        if tokenizer is not None:
            x = tokenizer.decode(x)
        xx = [str(t[1:]) for t in x.split(' ') if (t[0]=='u')]
        try:
            m = bytes(map(int, xx)).decode('utf-8', 'ignore')
        except:
            print('error', x, xx);exit()
        return m
    ...
    for sample in progress:
        ...
        for i, sample_id in enumerate(sample['id'].tolist()):
            ...
            # print('source language', sample_id, src_str)
            src_str = decode_fn_src(src_str)
            # print('source language 1', src_str)

            # if sample_id == 1471:
            #     exit()

            if has_target:
                target_str = decode_fn_tgr(target_str)
            ...
            if not args.quiet:
                ...
                if has_target:
                    ...
                    # output_file.write('T-{}\t{}'.format(sample_id, target_str))
            ...
            for j, hypo in enumerate(hypos[i][:args.nbest]):
                ...
                detok_hypo_str = decode_fn_tgr(hypo_str)
                if not args.quiet:
...
def cli_main():
    ...
    print('result_path', args.results_path)
    main(args)
