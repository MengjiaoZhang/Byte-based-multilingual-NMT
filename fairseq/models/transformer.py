...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        self.no_embed = args.no_embed
        if self.no_embed:
            self.embed_scale = nn.Parameter(torch.Tensor([0]))
            nn.init.constant_(self.embed_scale, math.sqrt(embed_dim))
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        if self.no_embed:
            ...
            self.out_embed_scale = nn.Parameter(torch.Tensor([0]))
            nn.init.constant_(self.out_embed_scale, math.sqrt(embed_dim))

            self.layers[-1].dropout_module.p = 0
    ...
    def output_layer(self, features):
        ...
        if self.adaptive_softmax is None:
            ...
            if self.no_embed:
                a = self.out_embed_scale * self.output_projection(features)
                return a
...
def base_architecture(args):
    ...
    args.no_embed = getattr(args, "no_embed", False)
